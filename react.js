...
useState возвращает массив ровно с двумя значениями: текущее состояние (state), которое во время первого рендеринга будет соответствовать initialStateвы и функция (setState),
которая позволяет обновить состояние до другого значения и запустить повторный рендеринг.

//useRef

useRef - это React Hook для добавления ссылки на свой компонент.
...
Для получения доступа к текущему значению ссылки используется свойство ref.current. При изменении значения ссылки повторный рендеринг не вызывается.
...
// Бесконечная загрузка, виртуализация списков, нет смысла создавать супер вложенную структуру и верску
//2) React-оптимизации
// юзКолбэк, юзМемо, мемоХОК, юзДефередВэлью (приоритеты реакт-задач), саспенс + лэйзи
//3) Сетевая оптимизация
// Дебаунс, сроттл, аккуратно делать запросы, проверять зависимости эффектов, думать где можно использовать поллинг, где можно использоватьсокеты
